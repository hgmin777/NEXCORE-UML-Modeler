...
public class StringMatcher {
    ...
    public StringMatcher.Position find(String text, int start, int end) {
        ...
        for (i = 0; i < segCount && curPos < end; ++i) {
            current = fSegments[i];
            nextMatch = regExpPosIn(text, curPos, end, current);
            if (nextMatch < 0) {
                return null;
            }
            if (i == 0) {
    ...
    public boolean match(String text, int start, int end) {
        ...
        int currentMatch, k = 0;
        while (i < segCount) {
            current = fSegments[i];

            k = current.indexOf(fSingleWildCard);
            if (k < 0) {
    ...
    protected boolean regExpRegionMatches(String text, int tStart, String p, int pStart, int plen) {
        char tchar, pchar = 0;
        while (plen-- > 0) {
            tchar = text.charAt(tStart++);
            pchar = p.charAt(pStart++);
