...
import nexcore.tool.uml.core.CoreConstant;
import nexcore.tool.uml.core.exception.UMLRuntimeException;
import nexcore.tool.uml.core.message.UMLMessage;
import nexcore.tool.uml.core.regexp.RegExp;
import nexcore.tool.uml.core.regexp.RegExpParameter;
 ...
 * <li>설  명 : StringUtil</li>
 * <li>작성일 : 2007. 04. 02.</li>
 * <li>작성자 : 기술전략팀 표준 업무 담당자 </li>
 * <li>2009.06.04 nexbuild.foundation.core 플러그인에서 복사해옴</li>
 * </ul>
 */
public class StringUtil {
     ...
     * 알 수 없는 인코딩 형식입니다.
     */
    //    private static String unknownEncoding = "Unknown encoding type"; //$NON-NLS-1$
    /**
     * 파라메터에 문제가 있습니다.
     */
    //    private static String illegalArgument = "Invalid parameter"; //$NON-NLS-1$
    /**
     * 입력된 String 문자열이 null 혹은 빈문자열인지 확인한다.
     * 
     * <pre>
    ...
    /**
     * 입력된 StringBuffer 문자열이 null 혹은 빈문자열인지 체크한다. 만일 null인 경우
    ...
    public static String kscToAsc(String str) {
        try {
            return str != null ? new String(str.getBytes("KSC5601"), "8859_1") : str; //$NON-NLS-1$ //$NON-NLS-2$
     ...
     * 0xc8fe(힝)까지이다.<br/>
     * 완성형 한글의 코드 형태는 완성된 한글 한 글자 당 2바이트 크기의 코드를 부여하여 각 글자별로 코드값을 지정하는 방식이다.
     * 
     * @param str
     *            8859_1로 인코딩된 문자열
    ...
    public static String ascToKsc(String str) {
        ...
        try {
            return str != null ? new String(str.getBytes("8859_1"), "KSC5601") : str; //$NON-NLS-1$ //$NON-NLS-2$
        } catch (UnsupportedEncodingException e) {
            throw new UMLRuntimeException(e);
        }
    }
    ...
    public static void verifyStringParameter(String... parameters) {
        String message = UMLMessage.MESSAGE_PARAMETER_STRING_NULL;
        for (String parameter : parameters) {
            if (parameter == null) {
                throw new UMLRuntimeException(message);
            } else if (parameter.equals("")) { //$NON-NLS-1$
                throw new UMLRuntimeException(UMLMessage.MESSAGE_PARAMETER_STRING_EMPTY);
            }
        }
    }
    ...
    public static void verifyObjectParameter(Object... parameters) {
        String message = UMLMessage.MESSAGE_PARAMETER_OBJECT_NULL;
    ...
    /**
     * String을 원하는 길이(byte 단위)로 줄여 마지막 접미사를 붙여 반환한다.<br/>
     * 접미사의 길이도 길이에 포함되며, 원하는 크기가 접미사의 길이보다 작으면 BaseRuntimeException 던진다. 주어진
     * 인코딩 기준으로 byte[]를 얻은 후 이를 byte 단위로 나누며, 나누는 과정에 깨지는 문자는 버린다.
     * 
     * <pre>
    ...
    public static String abbreviate(String content, int maxWidth, String enc, String suffix) {
        ...
        if (maxWidth < suffix.length())
            throw new UMLRuntimeException(new IllegalArgumentException());
    ...
    public static String nullStringCheck(String source) {
        return nullStringCheck(source, ""); //$NON-NLS-1$
    }
    ...
    public static String replaceDbNull(String source) {
        if (source == null || source.length() == 0 || "null".equals(source)) { //$NON-NLS-1$
            return ""; //$NON-NLS-1$
        } else {
            return source;
        }
    ...
    public static String formatNumber(String targetVal, String type) {
        ...
        if (targetVal != null) {
            ...
            if ("INT".equals(type)) { // 순수정수형 //$NON-NLS-1$

                intVal = new Integer(targetVal).intValue();

                DecimalFormat dfInt = new DecimalFormat("#,##0"); //$NON-NLS-1$

                rtnVal = dfInt.format(intVal);

            }
            if ("FINT".equals(type)) { // 더블형에서 정수형 //$NON-NLS-1$
            ...
            } else if ("DBL".equals(type)) { // 순수 더블형 //$NON-NLS-1$

                dblVal = new Double(targetVal).doubleValue();

                DecimalFormat dfDbl = new DecimalFormat("#,##0.00"); //$NON-NLS-1$

                rtnVal = dfDbl.format(dblVal);

            } else if ("IDBL".equals(type)) { // 정수가 OVERFLOW(LONG TYPE 정수) //$NON-NLS-1$

                dblVal = new Double(targetVal).doubleValue();

                DecimalFormat dfDbl = new DecimalFormat("#,##0"); //$NON-NLS-1$

                rtnVal = dfDbl.format(dblVal);

            } else if ("DDBL".equals(type)) { // 더블형이 소수점 4자리인경우 //$NON-NLS-1$

                dblVal = new Double(targetVal).doubleValue();

                DecimalFormat dfDbl = new DecimalFormat("#,##0.0000"); //$NON-NLS-1$

                rtnVal = dfDbl.format(dblVal);
            } else if ("DDBL1".equals(type)) { // 더블형이 소수점 1자리인경우 //$NON-NLS-1$

                dblVal = new Double(targetVal).doubleValue();

                DecimalFormat dfDbl = new DecimalFormat("#,##0.0"); //$NON-NLS-1$

                rtnVal = dfDbl.format(dblVal);
            } else if ("DDBL3".equals(type)) { // 더블형이 소수점 3자리인경우 //$NON-NLS-1$

                dblVal = new Double(targetVal).doubleValue();

                DecimalFormat dfDbl = new DecimalFormat("##,###,###,##0.000"); //$NON-NLS-1$

                rtnVal = dfDbl.format(dblVal);
            } else if ("DDBL6".equals(type)) { // 더블형이 소수점 6자리인경우 //$NON-NLS-1$

                dblVal = new Double(targetVal).doubleValue();

                DecimalFormat dfDbl = new DecimalFormat("##,###,###,##0.000000"); //$NON-NLS-1$

                rtnVal = dfDbl.format(dblVal);
            } else if ("DDBL7".equals(type)) { // 더블형이 소수점 7자리인경우 //$NON-NLS-1$

                dblVal = new Double(targetVal).doubleValue();

                DecimalFormat dfDbl = new DecimalFormat("##,###,###,##0.0000000"); //$NON-NLS-1$

                rtnVal = dfDbl.format(dblVal);
            } else if ("INT4".equals(type)) { // 정수지만 앞에 영이 붙는것 예) 0025 //$NON-NLS-1$

                int diff = 4 - targetVal.trim().length();
                rtnVal = targetVal;
                for (int i = 0; i < diff; i++) {
                    rtnVal = "0" + rtnVal; //$NON-NLS-1$
                }
            }
        }
    ...
    public static String getFileName(String fullFileName) {
        try {
            return getFileName(fullFileName, System.getProperty("file.separator")); //$NON-NLS-1$
        } catch (Exception ex) {
            return null;
        }
    }
    ...
    public static boolean isEmptyAfterTrim(String string) {
        return string == null || string.trim().equals("") ? true : false; //$NON-NLS-1$
    }
    ...
    public static boolean isDoubleNumber(String digit) {
        ...
        Pattern pattern = Pattern.compile(CoreConstant.DOUBLE_NUMBER_PATTERN);
    ...
    public static boolean isHangul(char c) {
        ...
        return (unicodeBlock.equals("HANGUL_JAMO") || unicodeBlock.equals("HANGUL_SYLLABLES") || unicodeBlock.equals("HANGUL_COMPATIBILITY_JAMO")) //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        ? true : false;
    }
    ...
    public static boolean isFormattedString(String string, int patternId) {
        ...
        switch (patternId) {
            case CoreConstant.IP_ADDRESS:
                return isFormattedString(string, CoreConstant.IP_PATTERN);

            case CoreConstant.EMAIL_ADDRESS:
                return isFormattedString(string, CoreConstant.EMAIL_PATTERN);

            case CoreConstant.WEB_ADDRESS:
                return isFormattedString(string, CoreConstant.WEB_ADDRESS_PATTERN);

            case CoreConstant.ZIP_CODE:
                return isFormattedString(string, CoreConstant.ZIP_CODE_PATTERN);

            case CoreConstant.SS_NO:
                return isFormattedString(string, CoreConstant.SSNO_PATTERN);

            case CoreConstant.ER_NO:
                return isFormattedString(string, CoreConstant.ERNO_PATTERN);

            case CoreConstant.TEL_NO:
                return isFormattedString(string, CoreConstant.TELNO_PATTERN);
    ...
    public static String encode(String string, String fromEncoding, String toEncoding) {
        StringUtil.isEmpty(string);
        StringUtil.isEmpty(fromEncoding);
        StringUtil.isEmpty(toEncoding);
    ...
    /**
     * 정수 값을 boolean 값으로 변환한다. c언어에서 활용하는 방식과 동일하다.<br/>
     * 0 -&gt; false, !0 -&gt; true
     * 
     * <pre>
    ...
    public static String toFormattedString(String string, int patternId) {
        ...
        switch (patternId) {
            ...
            default:
                throw new UMLRuntimeException("Invalid format"); //$NON-NLS-1$
        }
    }
    ...
    private static String toFormattedZipCode(String string) {
        ...
        if (string.length() != 6 || !isDigit(string)) {
            throw new UMLRuntimeException("It is not zip code"); //$NON-NLS-1$
        }
    ...
    private static String toFormattedTelNo(String string) {
        ...
        if (length < 9 || length > 11 || !isDigit(string)) {
            throw new UMLRuntimeException("Invalid Telephone No format"); //$NON-NLS-1$
        }
    ...
    private static String toUnformattedString(String string) {
        return string == null ? null : string.replaceAll("[^0-9]", ""); //$NON-NLS-1$ //$NON-NLS-2$
    }
    ...
    public static String toUnformattedNumber(String formattedNumber) {
        StringUtil.isEmpty(formattedNumber);
    ...
    public static boolean isAlphabet(String text) {
        ...
        if (Pattern.matches(CoreConstant.ALPHABET_PATTERN, text)) {
            return true;
    ...
    public static boolean isAlphabetOrDigit(String text) {
        StringUtil.isEmpty(text);
        if (Pattern.matches(CoreConstant.ALPHABET_DIGIT_PATTERN, text)) {
            return true;
    ...
    public static boolean isAlphabetOrDigitOrUnderbar(String text) {
        StringUtil.isEmpty(text);
        if (Pattern.matches(CoreConstant.ALPHABET_DIGIT_UNDERBAR_PATTERN, text)) {
            return true;
    ...
    public static boolean isEmailPattern(String text) {
        StringUtil.isEmpty(text);
        if (Pattern.matches(CoreConstant.EMAIL_PATTERN, text)) {
            return true;
        }
        return false;
    }
    ...
    /**
     * Camel 형태의 String으로 변환한다. 일반적으로 SQL 컬럼 명이 ABC_DEF인 경우, abcDef 로 변경해준다. *
     * 
     * <pre>
     * StringUtil.convertToDBColumnString(&quot;CARMEL_NOTATION_STRING&quot;);
     * result: &quot;carmelNotationString&quot;
     * </pre>
     * 
     * @param columnString
     *            입력 문자열
     * @return String Camel 타입으로 변환된 문자열
     */
    @SuppressWarnings("unchecked")
    public static String convertToCamelString(String columnString) {
        String result = ""; //$NON-NLS-1$
        String temp = columnString.toLowerCase();
        int startIndex = 0;
        List<RegExpParameter> patternInfoList = RegExp.findPatternList(temp, "_"); //$NON-NLS-1$

        RegExpParameter param = null;
        for (Iterator i = patternInfoList.iterator(); i.hasNext();) {
            param = (RegExpParameter) i.next();
            result = result + temp.substring(startIndex, param.getStartPos());
            if (param.getEndPos() == temp.length()) {
                // 맨 뒤에 _만 있고 문자가 없는 경우 무시하고 넘어감...
                // 예) TEST_123_ 이와같은 경우
                break;
            }
            result = result + temp.substring(param.getEndPos(), param.getEndPos() + 1).toUpperCase();
            startIndex = param.getEndPos() + 1;
        }
        result = result + temp.substring(startIndex, temp.length());
        return result;
    }
    ...
    public static int getLocWithComment(String contents) {
        ...
        if (contents == null || "".equals(contents)) { //$NON-NLS-1$
            return 0;
        }
        ...
        try {
            while ((lineString = br.readLine()) != null) {
                if ("".equals(lineString.trim())) { //$NON-NLS-1$
                    continue;
                }
    ...
    public static int getLocWithoutComment(String contents) {
        ...
        int tempEndIndex = 0;
        while (sb.indexOf("/**") != -1 || sb.indexOf("/*") != -1 || sb.indexOf("//") != -1 || loopCount-- > 0) { //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            if (sb.indexOf("/**") != -1) { //$NON-NLS-1$
                startIndex = sb.indexOf("/**"); //$NON-NLS-1$
                endIndex = sb.indexOf("*/", startIndex); //$NON-NLS-1$
                if (endIndex == -1) {
                    sb.delete(startIndex, sb.length());
                } else {
                    tempEndIndex = endIndex + 2; // +2 하는 이유 : "*/" 만큼 뒤로
                    // 이동해야 하므로.
            ...
            } else if (sb.indexOf("/*") != -1) { //$NON-NLS-1$
                startIndex = sb.indexOf("/*"); //$NON-NLS-1$
                endIndex = sb.indexOf("*/", startIndex); //$NON-NLS-1$
                if (endIndex == -1) {
                    sb.delete(startIndex, sb.length());
    ...
    public static boolean isFileForBuild(String fileName, String filePath) {
        ...
        if (filePath.indexOf("CVS") > -1 || filePath.indexOf("cvs") > -1 || filePath.indexOf("SVN") > -1 //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            || filePath.indexOf("svn") > -1) { //$NON-NLS-1$
            return false;
        }
        return true;
    }
    ...
    public static boolean hasStringInArray(String[] stringArray, String findString) {
        if (stringArray == null || stringArray.length == 0 || findString == null || "".equals(findString)) { //$NON-NLS-1$
            return false;
        }
    ...
    /**
     * 현재 실행되고 있는 OS에 적합한 Line separator를 리턴한다. 주의 : Eclipse내에서만 실행되는 메소드임
     * 
     * @return String
     */
    ...
    /**
     * 파일명에 포함되지 말아야 할 문자열을 인자로 넘긴 치환 문자열로 대체한 파일명 반환
     * 
     * @param fileName
     * @param replaceString
     * @return String
     */
    public static String getProperFileName(String fileName, String replaceString) {
        // \\p{Punct} : 구두점 캐릭터: ! "#$%&'()*+,-. /:;<=>? @[\]^_`{|}~
        // \\p{Space} : 공백 캐릭터: [ \t\n\x0B\f\r]

        String patternString = "\\p{Punct}|\\p{Space}";
    ...
    /**
     * 용어사전 영문 변환시 포함되지 말아야 할 문자열 제외하고 반환
     * 
     * @param fileName
     * @param replaceString
     * @return String
     */
    public static String getGlossaryTranslationName(String fileName, String replaceString) {
        ...
        for (char c : chars) {
            String gloss = String.valueOf(c);
            if ((c != '_') && (c != '-') && (c != ' ')) {
                Matcher matcher = pattern.matcher(gloss);
                gloss = matcher.replaceAll("");
            }
            result = result + gloss;
        }
