 ...
 * <li>설 명 : ProvidedInterfacesEditPart</li>
 * <li>작성일 : 2009. 12. 7.</li>
 * <li>작성자 : 강경구</li>
 * </ul>
 */
public class ProvidedInterfacesEditPart extends ScrollableEditPart {
    ...
    protected List getModelChildren() {
        NotationNode providedInterfacesModel = (NotationNode) this.getModel();
        List<NotationNode> list = new ArrayList<NotationNode>();
        Component component = (Component) providedInterfacesModel.getUmlModel();
        if (component == null) {
            component = (Component) ((AbstractNode) providedInterfacesModel.getParent()).getUmlModel();
        }
        if (null == component) {
            return list;
        }
        ...
        if (component instanceof NamedElement) {
            if (component.getName() != null && !component.getName().equals(UICoreConstant.EMPTY_STRING)) {
                ...
                nameofUML.setParent(providedInterfacesModel);

                list.add(nameofUML);
            }
        }

        EList<Interface> interfaceList = component.getProvideds();
        for (Interface umlInterface : interfaceList) {

            NotationNode operationModel = UMLDiagramFactory.eINSTANCE.createNotationNode();
            operationModel.setNodeType(NodeType.PROVIDED_INTERFACE);
            operationModel.setParent(providedInterfacesModel);
